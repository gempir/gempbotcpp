...
TwitchIRC::TwitchIRC(UFC32 nick, UFC32 usr, UFC32 pass, UFC32 addr, U32 port, UFC32 channel) :
    ...
    Lib::writeToLog("gempbotLog.txt", "{C++} Establishing TwitchIRC Instance");
    //Create the buffer
    ...
    if(!_socketObj) {
        ...
        Lib::writeToLog("gempbotLog.txt", "{C++} Failed to connect to " + string(addr)+ ".");
        return;
    }
    serverAddr = addr;
    serverPort = port;
    ...
    Lib::writeToLog("gempbotLog.txt", "{C++} Establishing TwitchIRC Login Token");
    ...
    if(response.find("Welcome") == string::npos) {
        ...
        Lib::writeToLog("gempbotLog.txt", "{Twitch} We did not recieve a welcome message");
        CloseSocket();
    }
    else {
        ...
        Lib::writeToLog("gempbotLog.txt", "{Twitch} Connected to TwitchIRC, connecting to channel '#" + string(channel) + "'.");
        ...
        SendChatMessage("gempbot Now Connected to channel...");
...
TwitchIRC::~TwitchIRC() {
    Lib::writeToLog("gempbotLog.txt", "{C++} Calling ~TwitchIRC(), closing program...\n\n");
    CloseSocket();
}
...
void TwitchIRC::Update() {
    ...
    if(response.size()) {
        ...
        if(response.find("PRIVMSG") != string::npos) {
        	...
        	//Lib::writeToLog("gempbotLog.txt", "{Twitch} UIID Response '" + response + "'.");
        }
    }
}
...
bool TwitchIRC::SendChatMessage(UFC32 message) {
	...
	Lib::writeToLog("gempbotLog.txt", "{Twitch} Sending Message " + Lib::formatForPrint(message) + "...");
